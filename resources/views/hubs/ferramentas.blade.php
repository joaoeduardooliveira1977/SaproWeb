...
<div class="hub-kpis" style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:20px;">
    ...
    <a href="{{ $k['route'] }}" style="text-decoration:none;">
        <div style="background:{{ $k['bg'] }};border-radius:14px;padding:22px 20px;color:#fff;cursor:pointer;transition:transform .15s,box-shadow .15s;box-shadow:0 4px 15px rgba(0,0,0,.15);"
            ...
            <div style="font-size:{{ strlen((string)$k['val']) > 8 ? '18' : '28' }}px;font-weight:800;margin-bottom:4px;letter-spacing:-1px;">{{ $k['val'] }}</div>
...
<div style="background:var(--white);border:1.5px solid var(--border);border-radius:12px;padding:20px;">
    ...
    <div class="hub-ferramentas" style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;">
        ...
        <a href="{{ $f['route'] }}" style="text-decoration:none;">
            <div style="display:flex;align-items:center;gap:14px;padding:18px;border-radius:12px;background:{{ $f['bg'] }};border:1.5px solid {{ $f['cor'] }}22;transition:all .15s;cursor:pointer;"
                onmouseover="this.style.borderColor='{{ $f['cor'] }}';this.style.transform='translateY(-2px)';this.style.boxShadow='0 4px 12px rgba(0,0,0,.08)'"
                onmouseout="this.style.borderColor='{{ $f['cor'] }}22';this.style.transform='';this.style.boxShadow=''">
                <div style="width:44px;height:44px;border-radius:12px;background:#fff;color:{{ $f['cor'] }};display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px rgba(0,0,0,.08);">
                    {!! $f['svg'] !!}
                </div>
                <div>
                    <div style="font-size:14px;font-weight:700;color:{{ $f['cor'] }};">{{ $f['label'] }}</div>
                    <div style="font-size:11px;color:var(--muted);margin-top:3px;line-height:1.4;">{{ $f['desc'] }}</div>
                </div>
            </div>
        </a>
        @endforeach
