@extends('layouts.app')
@section('page-title', 'Central de Ferramentas')
...
<div class="hub-kpis" style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:20px;">
    ...
    <a href="{{ $k['route'] }}" style="text-decoration:none;">
        <div style="background:#fff;border:1.5px solid var(--border);border-radius:10px;padding:16px;display:flex;align-items:center;gap:12px;transition:border-color .15s,transform .15s;"
            onmouseover="this.style.transform='translateY(-2px)';this.style.borderColor='{{ $k['cor'] }}'"
            onmouseout="this.style.transform='';this.style.borderColor='var(--border)'">
            <div style="width:40px;height:40px;border-radius:8px;background:{{ $k['bg'] }};color:{{ $k['cor'] }};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                {!! $k['svg'] !!}
            </div>
    ...
    <div style="display:flex;align-items:flex-start;gap:16px;">
        ...
        <div style="flex:1;">
            ...
            <div style="font-size:13px;color:var(--muted);line-height:1.7;">
                @if($ultimaConsulta)
                    Última consulta ao DATAJUD <strong style="color:var(--primary);">{{ $ultimaConsulta->concluido_em->locale('pt_BR')->diffForHumans() }}</strong>
                    com <strong style="color:#059669;">{{ $ultimaConsulta->novos_total }} andamento(s) novo(s)</strong> encontrado(s).
                    @if($publicacoesNaoLidas > 0)
                        Há <strong style="color:#d97706;">{{ $publicacoesNaoLidas }} publicação(ões)</strong> aguardando vinculação a processos.
                    @endif
                @else
            ...
            <div style="display:flex;flex-wrap:wrap;gap:16px;margin-top:12px;font-size:12px;color:var(--muted);">
                <span style="display:flex;align-items:center;gap:5px;">
                    ...
                    <strong style="color:#2563a8;">{{ $totalProcessosMonitorados }}</strong>&nbsp;processos monitorados
                </span>
                <span style="display:flex;align-items:center;gap:5px;">
                    <svg width="8" height="8" viewBox="0 0 24 24" fill="#059669" stroke="none"><circle cx="12" cy="12" r="10"/></svg>
                    <strong style="color:#059669;">{{ $totalAndamentosHoje }}</strong>&nbsp;andamentos importados hoje
                </span>
                @if($publicacoesNaoLidas > 0)
                <span style="display:flex;align-items:center;gap:5px;">
                    <svg width="8" height="8" viewBox="0 0 24 24" fill="#d97706" stroke="none"><circle cx="12" cy="12" r="10"/></svg>
                    <strong style="color:#d97706;">{{ $publicacoesNaoLidas }}</strong>&nbsp;publicações não vinculadas
                </span>
                @endif
            </div>
        </div>
    ...
    <div class="hub-ferramentas" style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;">
        ...
        $ferramentas = [
            ...
            [
                ...
                'desc'  => 'Propostas de honorários com PDF e integração CRM',
                'cor'   => '#0891b2',
                'bg'    => '#f0f9ff',
                'badge' => 'Novo',
                'route' => route('orcamentos'),
                'svg'   => '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>',
            ],
            [
        ...
        <a href="{{ $f['route'] }}" style="text-decoration:none;">
            <div style="background:#fff;border:1.5px solid var(--border);border-radius:10px;padding:16px;transition:border-color .15s,transform .15s;position:relative;min-height:148px;"
                onmouseover="this.style.borderColor='{{ $f['cor'] }}';this.style.transform='translateY(-2px)'"
                onmouseout="this.style.borderColor='var(--border)';this.style.transform=''">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;">
                    <div style="width:38px;height:38px;border-radius:8px;background:{{ $f['bg'] }};color:{{ $f['cor'] }};display:flex;align-items:center;justify-content:center;">{!! $f['svg'] !!}</div>
                    <span style="background:{{ $f['cor'] }};color:#fff;padding:3px 8px;border-radius:99px;font-size:11px;font-weight:800;">{{ $f['badge'] }}</span>
                </div>
                <div style="font-size:13px;font-weight:700;color:var(--text);margin-bottom:5px;">{{ $f['label'] }}</div>
                <div style="font-size:11px;color:var(--muted);line-height:1.5;margin-bottom:8px;">{{ $f['desc'] }}</div>
                <div style="font-size:12px;font-weight:600;color:{{ $f['cor'] }};">Acessar ferramenta →</div>
            </div>
        </a>
        @endforeach
    </div>
