...
<div class="hub-bottom hub-grid-2" style="order:3;display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
    ...
    <div style="background:var(--white);border:1.5px solid var(--border);border-radius:12px;padding:20px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
            ...
            <a href="{{ route('prazos') }}" style="font-size:12px;color:var(--primary);text-decoration:none;">Ver todos...</a>
        </div>
        @if($prazosUrgentes->isEmpty())
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
            ...
            @endphp
            <a href="{{ route('prazos') }}" style="text-decoration:none;display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:8px;background:{{ $bg }};">
                <div style="width:36px;height:36px;border-radius:50%;background:{{ $cor }}22;color:{{ $cor }};font-size:11px;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                    {{ $dias < 0 ? abs($dias).'d' : ($dias === 0 ? 'Hj' : $dias.'d') }}
                </div>
                <div style="flex:1;min-width:0;">
                    <div style="font-size:13px;font-weight:600;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $prazo->titulo }}</div>
                    <div style="font-size:11px;color:var(--muted);">{{ $prazo->data_prazo->format('d/m/Y') }} · {{ $prazo->processo?->cliente?->nome ?? '—' }}</div>
                </div>
    ...
    <div style="background:var(--white);border:1.5px solid var(--border);border-radius:12px;padding:20px;">
        ...
        <div style="display:flex;flex-direction:column;">
            ...
            @endphp
            <a href="{{ route('processos.show', $mov->processo_id) }}" style="text-decoration:none;display:flex;align-items:flex-start;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);">
                <div style="width:32px;height:32px;border-radius:8px;background:{{ $corMov }}22;color:{{ $corMov }};font-size:12px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                    {{ $inicial }}
                </div>
                <div style="flex:1;min-width:0;">
                    <div style="font-size:12px;color:var(--text);font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ \Illuminate\Support\Str::limit($mov->descricao, 60) }}</div>
                    <div style="font-size:11px;color:var(--muted);margin-top:2px;">
                        <span style="color:var(--primary);font-weight:600;">{{ $mov->numero }}</span>
                        · {{ $mov->cliente_nome }}
                    </div>
                </div>
                <div style="font-size:10px;color:var(--muted);flex-shrink:0;white-space:nowrap;padding-top:2px;">
                    {{ \Carbon\Carbon::parse($mov->created_at)->locale('pt_BR')->diffForHumans() }}
                </div>
            </a>
            @endforeach
        </div>
