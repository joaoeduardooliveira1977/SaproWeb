    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar {
            ...
            height: 100vh; position: sticky; top: 0; overflow-y: auto; z-index: 100; flex-shrink: 0;
        }
        .sidebar-brand {
        ...
        .sidebar-brand-icon svg { width: 18px; height: 18px; stroke: #fff; fill: none; stroke-width: 2; }
        .sidebar-brand-text { font-size: 13px; font-weight: 700; color: #fff; line-height: 1.2; }
        .sidebar-brand-sub  { font-size: 10px; color: rgba(255,255,255,.4); text-transform: uppercase; letter-spacing: 1px; }

        .sidebar-section { padding: 12px 12px 4px; font-size: 10px; font-weight: 700; color: rgba(255,255,255,.3); text-transform: uppercase; letter-spacing: 1px; }

        .nav-item {
            display: flex; align-items: center; gap: 10px; padding: 10px 16px;
            font-size: 13px; font-weight: 500; color: rgba(255,255,255,.6);
            text-decoration: none; transition: all .15s; border-left: 3px solid transparent; cursor: pointer;
            background: none; border-top: none; border-right: none; border-bottom: none; width: 100%; text-align: left;
        }
        .nav-item:hover  { background: rgba(255,255,255,.05); color: #fff; }
        .nav-item.active { background: rgba(29,158,117,.15); color: var(--accent); border-left-color: var(--accent); }
        ...
        .nav-sep { height: 1px; background: rgba(255,255,255,.06); margin: 8px 0; }
        .nav-spacer { flex: 1; }
        ...
        .topbar-badge { background: #dc2626; color: #fff; font-size: 10px; font-weight: 700; padding: 3px 8px; border-radius: 4px; letter-spacing: .5px; }
...
<div class="layout">
    ...
    <aside class="sidebar">
        <div class="sidebar-brand">
            ...
            <div>
                <div class="sidebar-brand-text">Master Admin</div>
            </div>
        </div>

        <div class="sidebar-section">Painel</div>
        <a href="{{ route('master-admin.dashboard') }}"
           class="nav-item {{ request()->routeIs('master-admin.dashboard') ? 'active' : '' }}">
        ...
        <a href="{{ route('master-admin.tenants') }}"
           class="nav-item {{ request()->routeIs('master-admin.tenants*') ? 'active' : '' }}">
            <svg viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
            Tenants
        </a>
        <a href="{{ route('master-admin.alertas') }}"
           class="nav-item {{ request()->routeIs('master-admin.alertas') ? 'active' : '' }}">
            <svg viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
            Alertas
        </a>
        <a href="{{ route('master-admin.logs') }}"
           class="nav-item {{ request()->routeIs('master-admin.logs') ? 'active' : '' }}">
        ...
        <a href="{{ route('master-admin.infra') }}"
           class="nav-item {{ request()->routeIs('master-admin.infra') ? 'active' : '' }}">
        ...
        <a href="{{ route('super-admin.index') }}" class="nav-item">
