...
@section('page-title', 'Alertas')
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
        ...
        <button wire:click="carregarAlertas" class="btn btn-sm btn-outline">â†» Verificar agora</button>
    </div>
    ...
    <div class="card" style="margin-bottom:16px;border-left:4px solid #dc2626;">
        <div class="card-header" style="background:#fef2f2;">
            <span class="card-title" style="color:#dc2626;">ðŸ”´ CrÃ­ticos ({{ $criticos->count() }})</span>
        </div>
        ...
        <div class="alert-row" style="background:{{ $loop->even ? '#fef9f9' : '#fff' }};">
            ...
            <a href="{{ $a['link'] }}" class="btn btn-sm btn-danger">Ver â†’</a>
        </div>
        @endforeach
    </div>
    @endif
    ...
    <div class="card" style="border-left:4px solid #d97706;">
        <div class="card-header" style="background:#fffbeb;">
            <span class="card-title" style="color:#d97706;">ðŸŸ¡ Avisos ({{ $avisos->count() }})</span>
        </div>
        @foreach($avisos as $a)
        <div class="alert-row" style="background:{{ $loop->even ? '#fffdf5' : '#fff' }};">
            ...
            <a href="{{ $a['link'] }}" class="btn btn-sm btn-outline">Ver â†’</a>
        </div>
        @endforeach
    </div>
    @endif
