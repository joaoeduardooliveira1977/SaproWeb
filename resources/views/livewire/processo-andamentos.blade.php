<div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;flex-wrap:wrap;gap:10px;">
        ...
        <div class="card-actions">
            ...
            <button wire:click="consultarDatajud" wire:loading.attr="disabled"
                style="display:inline-flex;align-items:center;gap:6px;padding:6px 14px;background:#f0fdf4;border:1.5px solid #86efac;border-radius:8px;color:#16a34a;font-size:12px;font-weight:600;cursor:pointer;transition:background .15s;"
                onmouseover="this.style.background='#dcfce7'" onmouseout="this.style.background='#f0fdf4'">
                <svg wire:loading.remove wire:target="consultarDatajud" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/>
                    <path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/>
                </svg>
                <svg wire:loading wire:target="consultarDatajud" style="animation:spin .7s linear infinite;" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                <span wire:loading.remove wire:target="consultarDatajud">🔄 Consultar DATAJUD</span>
                <span wire:loading wire:target="consultarDatajud">Consultando...</span>
            </button>
            @if(tenant_pode('ia'))
    ...
    {{-- ── Resultado Consulta DATAJUD ── --}}
    @if($mostrarResultadoDatajud && $resultadoDatajud)
    <div style="display:flex;gap:10px;padding:12px 16px;background:{{ $novosAndamentosDatajud > 0 ? '#f0fdf4' : '#f0f9ff' }};border:1.5px solid {{ $novosAndamentosDatajud > 0 ? '#86efac' : '#93c5fd' }};border-radius:10px;margin-bottom:16px;align-items:center;">
        <div style="flex:1;font-size:13px;font-weight:600;color:{{ $novosAndamentosDatajud > 0 ? '#16a34a' : '#1d4ed8' }};">
            {{ $resultadoDatajud }}
        </div>
        <button wire:click="fecharResultadoDatajud"
            style="background:none;border:none;cursor:pointer;color:#94a3b8;padding:0;display:flex;align-items:center;flex-shrink:0;">
