  ...
  @if(empty($clientes))
    <div class="card" style="padding:0">
      <div class="empty-state">
        <div class="empty-state-icon"><svg aria-hidden="true" width="36" height="36" fill="none" stroke="var(--success)" stroke-width="1.5" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg></div>
        <div class="empty-state-title">Tudo em dia!</div>
        <div class="empty-state-sub">Nenhuma inadimplência encontrada com os filtros aplicados.</div>
      </div>
    </div>
  @else
          ...
          <div style="padding:14px 16px;display:flex;align-items:center;gap:12px;flex-wrap:wrap">
            ...
            <div style="display:flex;align-items:center;gap:8px;flex-shrink:0">
              ...
              <button wire:click="abrirContato({{ $parcelas[0]->id ?? 0 }}, {{ $cliente->cliente_id }})"
                ...
                <span style="display:inline-flex;align-items:center;gap:5px;"><svg aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg> Contato</span>
              </button>

              <button wire:click="abrirEmail({{ $cliente->cliente_id }})" class="btn btn-sm"
                      style="background:#7c3aed;color:#fff;border-color:#7c3aed">
                <span style="display:inline-flex;align-items:center;gap:5px;"><svg aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2-2-2z"/><polyline points="22,6 12,13 2,6"/></svg> E-mail</span>
              </button>
    ...
    <div class="modal" style="max-width:500px">
      <div class="modal-header">
        <span class="modal-title" style="display:inline-flex;align-items:center;gap:8px;">
          <span style="display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;background:#eff6ff;border-radius:6px;"><svg aria-hidden="true" width="14" height="14" fill="none" stroke="#2563eb" stroke-width="2" viewBox="0 0 24 24"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg></span>
          Registrar Tentativa de Contato
        </span>
        <button wire:click="$set('modalContato', false)" class="modal-close" aria-label="Fechar"><svg aria-hidden="true" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
      </div>
      ...
      <div class="form-field" style="margin:20px 20px 0">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:4px">
          @php
          $tiposContato = [
            'ligacao'    => ['svg' => '<svg aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 8.67 19.79 19.79 0 01.1 2.14 2 2 0 012.11 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 7.34a16 16 0 006.29 6.29l.75-.75a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>', 'label' => 'Ligação'],
            'whatsapp'   => ['svg' => '<svg aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>', 'label' => 'WhatsApp'],
            'email'      => ['svg' => '<svg aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2-2-2z"/><polyline points="22,6 12,13 2,6"/></svg>', 'label' => 'E-mail'],
            'reuniao'    => ['svg' => '<svg aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>', 'label' => 'Reunião'],
            'negociacao' => ['svg' => '<svg aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/></svg>', 'label' => 'Negociação'],
            'acordo'     => ['svg' => '<svg aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>', 'label' => 'Acordo'],
          ];
          @endphp
      ...
      @if($tipoContato === 'acordo')
        <div style="margin:8px 20px 0;padding:10px;background:#f0fdf4;border:1px solid #86efac;border-radius:6px;font-size:12px;color:#166534">
          <span style="display:inline-flex;align-items:center;gap:5px;"><svg aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg> Registrar como "Acordo" indica que houve negociação. Registre o pagamento separadamente quando for efetivado.</span>
        </div>
      @endif
  ...
  <div class="modal-backdrop" wire:click.self="$set('modalEmail', false)">
    <div class="modal" style="max-width:460px">
      <div class="modal-header">
        ...
        <button wire:click="$set('modalEmail', false)" class="modal-close" aria-label="Fechar"><svg aria-hidden="true" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
      </div>
  ...
  <div class="modal-backdrop" wire:click.self="$set('modalPagamento', false)">
    <div class="modal" style="max-width:400px">
      ...
      <div class="modal-footer">
        ...
        <button wire:click="registrarPagamento" class="btn btn-success"><span style="display:inline-flex;align-items:center;gap:5px;"><svg aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg> Confirmar Pagamento</span></button>
      </div>
    </div>
  </div>
  @endif
