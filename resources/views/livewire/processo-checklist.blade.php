        ...
        <div style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;border-radius:8px;
            ...
            <div style="flex:1;min-width:0;">
                ...
                <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:3px;">
                    ...
                    @endphp
                    <span style="font-size:11px;color:{{ $dlCor }};">
                        📅 {{ $dl->format('d/m/Y') }}
                        @if($vencida) <strong>(vencida)</strong> @endif
                    </span>
                    @endif
                    @if($t->resp_nome)
                    <span style="font-size:11px;color:#64748b;">👤 {{ $t->resp_nome }}</span>
                    @endif
                    @if($t->concluida && $t->concluida_em)
                    <span style="font-size:11px;color:#16a34a;">✓ {{ \Carbon\Carbon::parse($t->concluida_em)->format('d/m/Y H:i') }}</span>
                    @endif
                </div>
            </div>
            <div style="display:flex;gap:4px;flex-shrink:0;">
                <button wire:click="abrirEdicao({{ $t->id }})" title="Editar"
                    style="background:none;border:none;cursor:pointer;padding:3px 5px;font-size:14px;color:#64748b;">✏️</button>
