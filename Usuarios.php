...
class Usuarios extends Component
{
    ...
    public function salvar(): void
    {
        ...
        if ($this->usuarioId) {
            $sql = "UPDATE usuarios SET nome=?, login=?, email=?, perfil=?, telefone=?, ativo=?, updated_at=NOW()";
            $params = [$this->nome, $this->login, $this->email ?: null, $this->perfil, $this->telefone ?: null, $this->ativo];
            ...
            DB::insert("
                INSERT INTO usuarios (nome, login, email, senha, perfil, telefone, ativo, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            ", [
    ...
    public function render()
    {
        ...
        if ($this->busca) {
            $where .= " AND (nome ILIKE ? OR login ILIKE ?)";
            $params[] = "%{$this->busca}%";
            $params[] = "%{$this->busca}%";
        }
        ...
        $usuarios = DB::select("
            SELECT * FROM usuarios {$where} ORDER BY nome
        ", $params);
